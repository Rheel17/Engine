 ...
 */
#include "_GLVertexArray.h"

namespace rheel {

_GLVertexArray::VertexAttribute::VertexAttribute(GLuint index, GLint size, GLenum type, GLsizei stride, GLsizeiptr offset, bool normalize) :
...
_GLVertexArray::VertexAttribute::VertexAttribute() :
		_index(0), _size(0), _type(0), _stride(0), _offset(0), _normalize(false) {}

GLsizei _GLVertexArray::VertexAttribute::_ByteSize() const {
	switch (_type) {
...
GLuint _GLVertexArray::ID() const {
	return _id;
}

void _GLVertexArray::Bind() const {
	_GL::BindVertexArray(_id);
}

void _GLVertexArray::SetVertexAttributes(const _GLBuffer& buffer, const std::vector<VertexAttribute>& attributes, bool instanced) {
	if (buffer.Target() != _GL::BufferTarget::ARRAY) {
	...
	_GL::ClearBufferBinding(_GL::BufferTarget::ARRAY);
	buffer.Bind();
...
void _GLVertexArray::SetVertexIndices(const _GLBuffer& buffer) {
	if (buffer.Target() != _GL::BufferTarget::ELEMENT_ARRAY) {
	...
	// force that glBindBuffer() is actually called when this VAO is bound.
	_GL::ClearBufferBinding(_GL::BufferTarget::ELEMENT_ARRAY);
	buffer.Bind();
}

void _GLVertexArray::_SetVertexAttributes(const _GLBuffer& buffer, const std::vector<std::type_index>& attributeTypes, GLsizei stride, bool instanced) {
...
GLuint _GLVertexArray::_FirstUnusedIndex(GLuint consecutive) const {
	GLuint count = 0;
	GLuint startIndex = 0;
