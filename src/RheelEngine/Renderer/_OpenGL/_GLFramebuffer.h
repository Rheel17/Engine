...
public:
	 ...
	 */
	_GLFramebuffer(GLuint width, GLuint height, GLuint samples = 1,
	 ...
	 */
	_GLFramebuffer ResizedCopy(GLuint width, GLuint height);

	/**
	 * Binds this framebuffer to both the read and write buffer. Use
	 * _GLFramebuffer::ClearBinding() to reset the framebuffer to the default
	 * one.
	 */
	void Bind() const;
	 ...
	 * _GLFramebuffer::ClearBindingForReading() to reset the read framebuffer
	 * to the default one.
	 */
	 ...
	 * _GLFramebuffer::ClearBindingForWriting() to reset the write framebuffer
	 * to the derfault one.
	 */
	 ...
	 * and empty vector.
	 */
	const std::vector<_GLTexture2DMultisample>& MultisampleTextures() const;
	 ...
	 */
	const std::vector<_GLTexture2D>& Textures() const;

	/**
	 * Returns a vector of all the renderbuffers attached to this framebuffer.
	 */
	const std::vector<_GLRenderbuffer>& Renderbuffers() const;
...
private:
	...
	std::vector<_GLTexture2D> _textures;
	std::vector<_GLRenderbuffer> _renderbuffers;
	std::unordered_set<GLenum> _used_color_attachments;
	std::unordered_set<GLenum> _used_other_attachments;

	bool _is_multisampled;
	std::vector<_GLTexture2DMultisample> _multisample_textures;
