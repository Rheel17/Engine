...
SceneOld::SceneOld(const SceneDescription& description) {
...
const std::vector<std::unique_ptr<Script>>& SceneOld::Scripts() const {
	return _scripts;
}

Object& SceneOld::AddObject(const std::string& blueprintName, const vec3& position, const quat& rotation) {
...
void SceneOld::RemoveObject(Object& ptr) {
...
Light *SceneOld::GetLight(const std::string& lightName) {
...
const std::vector<std::string>& SceneOld::Lights() const {
	return _light_names;
}

void SceneOld::AddCamera(float fov, float near, float far, std::string name, vec3 position, vec3 rotation) {
	_cameras[name] = std::make_unique<PerspectiveCamera>(name, std::move(position), std::move(rotation), fov, near, far);
}

Camera *SceneOld::GetCamera(const std::string& cameraName) {
...
Object& SceneOld::_AddObject(const SceneDescription::_ObjectDescription& description) {
...
void SceneOld::_AddLight(const std::string& name, Light *light) {
...
void SceneOld::_FireObjectsEvent(Object::EventType event) {
